...
import { upsertFee } from "@/features/admin/adminCollegeService";
import { feeInputSchema } from "@/features/admin/adminCollegeSchemas";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = feeInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await upsertFee(id, parsed.data);
  if (!result.success) {
